...
from app.schemas.card_recommend_schema import (
...
"""
    resp = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "너는 카드사 앱의 AI 체크리스트 카테고리 분류기다.",
            },
...
def recommend_cards(schedule_id, user_id, conn, max_cards=4):
    with conn.cursor() as cur:
        ...
        cur.execute(
            """
            SELECT ci.checklist_item_id, ci.checklist_item_category
        ...
        if not checked_items:
            return {"userCardRecommend": [], "newCardRecommend": []}

        # 2. 항목 → 카테고리 분류 (LLM)
        categories = []
        for it in checked_items:
            cat = classify_category(it["checklist_item_category"])
        ...
        # 3. 보유 카드 확인
        cur.execute(
            "SELECT card_id FROM user_card WHERE user_id=%s AND is_deleted=b'0'",
            (user_id,),
        )
        ...
        # 8. 보유/신규 분리
        user_recs = [
            {"cardId": x["card_id"], "recommendContent": x["recommend_content"]}
            for x in top
            if x["is_user_card"]
        ]
        new_recs = [
            {"cardId": x["card_id"], "recommendContent": x["recommend_content"]}
            for x in top
            if not x["is_user_card"]
        ]

        return {"userCardRecommend": user_recs, "newCardRecommend": new_recs}


def run_task2(request: CardRecommendRequest) -> CardRecommendResponse:
    conn = get_connection()
    result = recommend_cards(
        request.scheduleId, bytes.fromhex(request.userUUID.replace("-", "")), conn
    )

    user_recs = [CardRecommendItem(**x) for x in result.get("userCardRecommend", [])]
    new_recs = [CardRecommendItem(**x) for x in result.get("newCardRecommend", [])]
    print(user_recs)
    print(new_recs)
    return CardRecommendResponse(
        userCardRecommend=user_recs,
        newCardRecommend=new_recs,
    )
