        ...
        "properties": {
            "items": {
                ...
                "items": {
                    "type": "object",
                    "properties": {
                        "checklist_item_category": {"type": "string", "enum": CATEGORIES},
...
def ensure_valid_categories(items):
    ...
    for it in items:
        ...
        if len(text) < 2:
            continue
        out.append({"checklist_item_category": cat, "checklist_item_content": text})
    return out[:7]
...
def run_task1():
    ...
    with conn.cursor() as cur:
        cur.execute("""
            ...
            FROM schedule
            WHERE schedule_is_checklist = b'1' AND (is_deleted IS NULL OR is_deleted = b'0')
        """)
        schedules = cur.fetchall()

        for s in schedules:
            sid, name = s["schedule_id"], s["schedule_name"]
            checklist = llm_generate_checklist(name)
            results.append({
                "schedule_id": sid,
                "schedule_name": name,
                "checklist": checklist
    ...
    with open("ai_checklist_results.json", "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print("[저장 완료] ai_checklist_results.json")
